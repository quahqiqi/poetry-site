...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    /* 搜索框与下拉列表 */
    .search-panel {{ position: absolute; top: 64px; right: 16px; width: 280px; background: var(--card); border: 1px solid var(--border); border-radius: 12px; padding: 8px; display: none; flex-direction: column; box-shadow: 0 10px 20px rgba(0,0,0,0.1); }}
    body.dark .search-panel {{ background: var(--card-dark); border-color: #333; }}
...
<body>
  ...
  <script>
    ...
    // ✨ 全新搜索框交互逻辑 ✨
    let allPoemsCache = null; // 用于缓存 JSON 数据

    document.getElementById('searchBtn').onclick = async () => {{ 
        const sp = document.getElementById('searchPanel'); 
        // 展开时设置为 display: flex，以便内部可以容纳结果列表
